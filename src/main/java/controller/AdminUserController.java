	 ...
	 @Override
	    ...
	    private ModelAndView handleDelete(HttpServletRequest request, HttpServletResponse response)
	        ...
	        try {
	            ...
	            int result = UsersService.getInstance().deleteUser(userNumber);
	            if (result > 0) {
	                System.out.println("[AdminUserController] 사용자 삭제 성공");
	            } else {
	                System.out.println("[AdminUserController] 사용자 삭제 실패");
	            }
