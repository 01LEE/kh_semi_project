...
public class UsersService {
    ...
    public List<UsersDTO> selectAllUsers() {
        // 관리자를 제외한 사용자 조회
        return mapper.selectAllUsersExcludeAdmin();
    }
    ...
    public int deleteUser(int userNumber) {
        System.out.println("[UsersService] deleteUser 호출, userNumber: " + userNumber);
        return mapper.deleteUserByUserNumber(userNumber);
    }

    public UsersDTO selectUserByNumber(int userNumber) {
        try {
            UsersDTO user = mapper.selectUserByNumber(userNumber);
            System.out.println("[UsersService] selectUserByNumber 결과: " + user);
            return user;
        } catch (Exception e) {
            System.out.println("[UsersService] selectUserByNumber 중 예외 발생: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
